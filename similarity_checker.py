...
class SimilarityChecker():
    ...
    def fit_value_type(self, attribute_key, schema_tuple):
        if attribute_key == "type" or attribute_key == "id":
            return None
        print(f"Fit 'value_type' of '{attribute_key}' in model: '{json.dumps(schema_tuple)}'")
        ...
        if self.db_helper.attribute_exists(attribute_key, _model, _subdomain, _domain, _version):
            ...
            _value_type = self._check_consistent_value_type(attribute_key, _model, _subdomain, _domain, _version)
            if _value_type:
                return _value_type
            # Controllo tra i value_type di s4c per trovare un nome simile
            _value_type = self._check_in_s4c_dict(attribute_key, _model, _subdomain, _domain, _version)
            if _value_type:
                return _value_type
        else:
            _value_type_2 = self._find_in_common_schemas(attribute_key, schema_tuple)
            if _value_type_2:
        ...
        if len(_common_attr) == 0:
            ...
            while len(_common_attr) > 0:
                ...
                if _cm_attr["checked"] == "True":
                    _res = self.s4c_dictionary.fit_value_type(_cm_attr["value_type"], True)
                    ...
                    _res = self._check_in_s4c_dict(attribute_key, _cm_model, _cm_subdomain, _cm_domain, _cm_version, returnList=True)
    ...
    def _check_in_s4c_dict(self, attribute_key, model, subdomain, domain, version, returnList=False):
        ...
        if isinstance(_pool, list):
            if len(_pool) == 1:
                ...
                self.db_helper.append_to_logs(model, subdomain, domain, version, attribute_key, self.get_message())
                return _instance
            elif len(_pool) > 1:
